...
VConnect 1.1 adds typed actions for a predefined connection request, recipient acceptance/decline, and approved public-place/time proposals. The reference operator host enforces five Anonymous Vibes and one VConnect request per authenticated passenger per sailing-local day. Production operators must apply the same limits server-side; client controls are not a security boundary.
