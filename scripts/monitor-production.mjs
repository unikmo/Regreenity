...
for (const [name,path] of checks) {
  const started=performance.now(), response=await fetch(`${origin}${path}`,{headers:{'User-Agent':'RegreenityReadinessMonitor/1.1'}}), body=await response.text()
  ...
  if(['site','demo','pilot','portal'].includes(name)&&!body.includes('Regreenity'))throw new Error(`${name} response is incomplete`)
