...
alter policy memberships_member_read on public.memberships using (user_id = auth.uid() or private.has_tenant_role(tenant_id,array['regreenity_admin','operator_admin']::public.member_role[]));
...
alter policy connectors_admin_read on public.connector_installations using (private.has_tenant_role(tenant_id,array['regreenity_admin','operator_admin']::public.member_role[]));
alter policy audit_admin_read on public.audit_events using ((tenant_id is null and exists(select 1 from public.memberships m where m.user_id=auth.uid() and m.role='regreenity_admin')) or (tenant_id is not null and private.has_tenant_role(tenant_id,array['regreenity_admin','operator_admin']::public.member_role[])));
...
alter policy product_configuration_admin_update on public.tenant_product_configuration using (private.has_tenant_role(tenant_id,array['regreenity_admin','operator_admin']::public.member_role[])) with check (private.has_tenant_role(tenant_id,array['regreenity_admin','operator_admin']::public.member_role[]));
alter policy product_configuration_admin_insert on public.tenant_product_configuration with check (private.has_tenant_role(tenant_id,array['regreenity_admin','operator_admin']::public.member_role[]));
