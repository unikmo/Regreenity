...
const buyerFaqs = [
  ...
  ['How are anonymous passenger vibes, safety and minors handled?', 'Anonymous Vibes are adult-only. The operator resolves a photographed receiver inside its identity zone, returns no name, rejects same-cabin/booking-group and duplicate attempts, caps each sender at eight vibes per sailing day, discards the capture after matching and delivers prepared compliments without sender identity or exact timing. A ninth attempt pauses sending until the next sailing day. Children cannot send, receive or be matched.'],
