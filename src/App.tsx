...
const seo: Record<View, { title: string; description: string; path: string }> = {
  overview: {
    title: 'Cruise Connection | Crew Recognition, Service Recovery & Passenger Engagement',
  ...
  passenger: {
    title: 'Passenger Experience | Cruise Connection',
  ...
  crew: {
    title: 'Crew Recognition for Cruise Lines | Cruise Connection',
  ...
  recovery: {
    title: 'Real-Time Service Recovery for Cruise Lines | Cruise Connection',
  ...
  commerce: {
    title: 'Social Commerce & Passenger Interests | Cruise Connection',
  ...
  dashboard: {
    title: 'Cruise-Line Dashboard | Cruise Connection',
  ...
  integration: {
    title: 'Cruise App Integration & Offline-First Architecture | Cruise Connection',
    description: 'See how Cruise Connection embeds in an existing cruise-line app, supports native proximity and ship-local operation without public internet, and hands commerce back to the host.',
    path: '/integration/',
  },
  pilot: {
    title: 'Cruise Connection Pilot & Commercial Framework',
    description: 'Design a one-ship Cruise Connection pilot with active passenger promotion, predefined KPIs and a working commercial model combining platform and attributable performance fees.',
    path: '/pilot/',
  },
  imprint: {
    title: 'Imprint | Cruise Connection',
    description: 'Legal operator and contact information for Cruise Connection, a product of PlanetHike OÜ.',
    path: '/imprint/',
  },
  privacy: {
    title: 'Privacy Policy | Cruise Connection',
    description: 'How Cruise Connection handles website, passenger, crew, proximity, service-recovery and commercial data.',
    path: '/privacy/',
  },
  terms: {
    title: 'Terms of Service | Cruise Connection',
    description: 'Terms governing the Cruise Connection website, demonstrations and passenger-facing module when deployed by a cruise-line partner.',
    path: '/terms/',
  },
  cookies: {
    title: 'Cookie & Local Storage Policy | Cruise Connection',
    description: 'How Cruise Connection uses essential browser storage, offline caching and cookies or similar technologies.',
    path: '/cookies/',
  },
}
    ...
    <main>
      <section className="hero-premium">
        ...
        <div className="section-shell hero-grid">
          <div className="hero-copy premium-copy">
            ...
            <p className="lead">Cruise Connection adds real-time crew recognition, private service recovery, safe passenger interaction and measurable social commerce to the cruise app you already have.</p>
          ...
          <div className="hero-stage" aria-label="Cruise Connection white-label passenger preview">
      ...
      <section className="crew-cinematic" id="crew-recognition">
        <div className="section-shell cinematic-grid">
          <div className="cinematic-copy">
            ...
            <p>Named crew recognition should not depend on a guest remembering a first name days later. Cruise Connection captures the moment, the sailing context and the reason for recognition while the experience is still fresh.</p>
      ...
      <section className="memory-section section-shell">
        ...
        <div className="memory-cards">
          <div className="share-card premium-share"><img className="memory-photo" src="/media/shareable-memory.jpg" alt="A passenger photographing a memorable coastal cruise destination" loading="lazy"/><small>CC-0826-17 · 2–9 AUG 2026</small><h3>Your Cruise Connection</h3><p>The positive moments you collected at sea.</p><div className="affirm-grid">{affirmations.map(a => <div key={a.label} className={`affirm ${a.tone}`}><b>{a.count}</b><span>{a.label}</span></div>)}</div><div className="share-footer"><span>Powered by Cruise Connection</span><button><Icon name="share" size={15}/> Share</button></div></div>
...
function PassengerModule() {
  ...
  const sharePassengerSummary = async () => {
    const text = 'My Cruise Connection · Sailing CC-0826-17 · 7× You made us laugh · 5× Great trivia teammate · 4× Enjoyed meeting you'
    try {
      if (navigator.share) await navigator.share({ title: 'My Cruise Connection', text })
      else if (navigator.clipboard) await navigator.clipboard.writeText(text)
    ...
    <main className="page section-shell">
      ...
      <section className="privacy-architecture proximity-privacy">
        ...
        <div className="privacy-grid">
          <article><span>01</span><b>Nearby is opt-in</b><p>Guests can make themselves visible to nearby Cruise Connection users for a limited period or while using the feature.</p></article>
      ...
      <div className="demo-grid refined passenger-demo-grid proximity-demo-grid">
        ...
        <div className="explain-stack">
          ...
          <article className="feature-large accent"><div className="number">04</div><div><h3>Meet in the real world</h3><p>Cruise Connection does not need to host a conversation. Guests meet onboard or can later move to the cruise line's chat or their own communication channel.</p></div></article>
      ...
      <section className="summary-preview"><div><div className="section-kicker light">END-OF-CRUISE MEMORY</div><h2>A positive artifact guests may actually want to share.</h2><p>Public sharing contains the recipient's affirmations—not the identities of the people who sent them unless separately permitted.</p></div><div className="share-card"><small>CC-0826-17 · 2–9 AUG 2026</small><h3>Your Cruise Connection</h3><p>The positive moments you collected at sea.</p><div className="affirm-grid">{affirmations.map(a => <div key={a.label} className={`affirm ${a.tone}`}><b>{a.count}</b><span>{a.label}</span></div>)}</div><div className="share-footer"><span>Powered by Cruise Connection</span><button onClick={sharePassengerSummary}><Icon name="share" size={16}/> {shareState}</button></div></div></section>
    </main>
  )
}
  ...
  return (
    <main className="page section-shell">
      ...
      <div className="crew-page-grid">
        ...
        <div className="crew-data-story"><div className="section-kicker">FROM THANK-YOU TO USEFUL SIGNAL</div><h2>Show total recognition with the context that makes it credible.</h2><p>Raw compliment totals alone are easy to misread. Cruise Connection separates breadth, depth and consistency.</p><div className="data-definition-grid"><div><b>84</b><span>Total recognition moments</span></div><div><b>61</b><span>Unique guests recognizing Ana</span></div><div><b>6/7</b><span>Sailing days with recognition</span></div></div><CrewRecognitionCard shareable /></div>
      </div>
...
function Commerce() {
  ...
  return (
    <main className="page section-shell">
      <div className="page-head editorial"><div><div className="section-kicker">PARTICIPATION & SOCIAL COMMERCE</div><h1>Use shared passenger intent to fill real experiences—<span>not push generic ads.</span></h1><p>Travellers voluntarily share cruise-relevant interests such as food, shopping, fitness, wellness and excursion styles. Cruise Connection uses that context—plus positive onboard connections—to surface relevant cruise inventory at the moment doing it together feels natural.</p></div><div className="pill coral"><Icon name="chart" size={15}/> Attribution-ready</div></div>

      <section className="interest-commerce-map">
        <div><div className="section-kicker light">INTEREST → CONNECTION → INVENTORY</div><h2>The same preference can improve both guest experience and revenue.</h2><p>The cruise line controls which interests map to which products. Cruise Connection provides the social context and group-intent signal.</p></div>
...
function IntegrationPage() {
  ...
  return (
    <main className="page section-shell integration-page">
      <div className="page-head editorial"><div><div className="section-kicker">CRUISE-APP INTEGRATION</div><h1>Add the module to the app guests already use. <span>Keep it useful even without public internet.</span></h1><p>The cruise-line app remains the trusted host. Cruise Connection adds the interaction layer through an embedded experience plus native bridge capabilities for identity, proximity, notifications and booking handoff.</p></div><div className="pill"><Icon name="layers" size={15}/> White-label · offline-first · host controlled</div></div>

      <section className="integration-flow-card">
        <div className="integration-sequence">
          <article><span>01</span><div><b>Module is available before the ship loses public internet</b><p>The cruise app pre-bundles or pre-caches the static module during app install/pre-sailing sync, or serves the shell from the ship-local network.</p></div></article><article><span>02</span><div><b>Guest opens Cruise Connection</b><p>The cruise line promotes the feature pre-sailing, at embarkation, from the daily program or through contextual prompts inside its existing app.</p></div></article>
          ...
          <article><span>07</span><div><b>Commerce returns to the host</b><p>Relevant purchase intent opens the cruise line's own inventory, pricing, checkout and payment route. Cruise Connection never needs to become the merchant.</p></div></article>
      ...
      <section className="offline-architecture">
        <div className="offline-arch-copy"><div className="section-kicker light">OFFLINE-FIRST SHIP ARCHITECTURE</div><h2>No public internet should not mean no Cruise Connection.</h2><p>The production design separates the <b>device layer</b>, the <b>ship-local layer</b> and the <b>external cloud layer</b>. The passenger should not care which path is currently available.</p></div>
      ...
      <section className="integration-guardrails">
        ...
        <div className="guardrail-list"><span><Icon name="shield" size={16}/> Short-lived signed launch token</span><span><Icon name="lock" size={16}/> Server-side validation</span><span><Icon name="users" size={16}/> Pseudonymous guest ID</span><span><Icon name="calendar" size={16}/> Sailing-scoped offline entitlement</span><span><Icon name="layers" size={16}/> Allowlisted host bridge</span><span><Icon name="check" size={16}/> No separate Cruise Connection login</span></div>
      </section>
    </main>
  )
}
...
function PilotPage() {
  ...
  return (
    <main className="page section-shell pilot-page">
      <div className="pilot-intro"><div className="section-kicker">PILOT DESIGN</div><h1>Prove Cruise Connection on <span>one ship.</span></h1><p>A useful pilot is deliberately bounded: selected sailings, defined passenger population, agreed feature set, active cruise-line promotion, predetermined measurement and a commercial decision date.</p><div className="pilot-principles"><div><b>01</b><span>One ship or defined sailing set</span></div><div><b>02</b><span>60–90 day target window</span></div><div><b>03</b><span>Predetermined KPIs</span></div><div><b>04</b><span>Active passenger promotion</span></div></div></div>
...
const LegalPage = ({ kind }: { kind: LegalKind }) => {
  ...
  if (kind === 'imprint') return (
    <main className="legal-page">
      <div className="legal-hero"><div className="section-kicker">LEGAL</div><h1>Imprint</h1><p>Legal operator information for Cruise Connection.</p><span>Last updated: {updated}</span></div>
      <div className="legal-layout"><aside className="legal-toc"><b>Operator</b><a href="#operator">Company information</a><a href="#contact">Contact</a><a href="#responsibility">Responsibility</a><a href="#ip">Intellectual property</a></aside><article className="legal-content">
        <section id="operator"><h2>1. Operator information</h2><p><strong>Cruise Connection</strong> is a product and trade name operated by <strong>PlanetHike OÜ</strong>.</p><dl className="legal-details"><div><dt>Legal entity</dt><dd>PlanetHike OÜ</dd></div><div><dt>Registered office</dt><dd>Järvevana tee 9, Tallinn, 11314, Estonia</dd></div><div><dt>Registry code</dt><dd>17233027</dd></div><div><dt>Register</dt><dd>Estonian Business Register</dd></div><div><dt>Legal representative</dt><dd>Tichi Mbanwie</dd></div></dl></section>
        <section id="contact"><h2>2. Contact</h2><p>Email: <a href="mailto:hello@planethike.org">hello@planethike.org</a><br/>Telephone: <a href="tel:+491634668380">+49 (0)163 466 8380</a></p><p>For privacy requests, use the same email address with the subject line <strong>“Privacy request – Cruise Connection”</strong>.</p></section>
        <section id="responsibility"><h2>3. Service and content responsibility</h2><p>PlanetHike OÜ is responsible for the Cruise Connection website and product materials. Cruise Connection is designed as white-label software for cruise lines. Unless a specific cruise-line partnership is expressly identified, demonstrations, ship names, passenger records, crew records, metrics and interface examples shown on this website are illustrative and do not imply affiliation with any cruise operator.</p><p>When Cruise Connection is deployed by a cruise-line partner, the cruise line remains responsible for its passenger relationship, ship operations, safety procedures, inventory, pricing, bookings, payments and physical onboard services.</p></section>
        <section id="ip"><h2>4. Intellectual property</h2><p>Unless otherwise stated, the Cruise Connection name, product concepts, interface designs, text, graphics, software, documentation, GAMMESO framework and other original materials are owned by PlanetHike OÜ or used under licence. Third-party marks remain the property of their respective owners.</p><p>Reproduction, modification, distribution, reverse engineering or commercial reuse beyond normal evaluation of the service requires prior written permission, except where applicable law provides otherwise.</p></section>
  ...
  if (kind === 'privacy') return (
    <main className="legal-page">
      ...
      <div className="legal-layout"><aside className="legal-toc"><b>Privacy</b><a href="#roles">Who controls data</a><a href="#data">Data we process</a><a href="#nearby">Nearby discovery</a><a href="#purposes">Purposes & bases</a><a href="#sharing">Sharing</a><a href="#retention">Retention</a><a href="#rights">Your rights</a></aside><article className="legal-content">
        <section><h2>1. Scope</h2><p>This policy explains how PlanetHike OÜ, operating Cruise Connection, handles personal data on this website and, where a cruise-line partner deploys Cruise Connection, within the embedded passenger and operational module.</p><p>The public website currently presents a product demonstration. A live cruise deployment will also provide cruise-line-specific privacy information reflecting that operator's systems, configuration and legal responsibilities.</p></section>
        <section id="roles"><h2>2. Who is responsible for your data?</h2><h3>Website and business enquiries</h3><p>PlanetHike OÜ is the data controller for information submitted directly to us through the Cruise Connection website, pilot enquiries, commercial communications and our own website security records.</p><h3>Passenger and crew module</h3><p>In a typical white-label deployment, the <strong>cruise line determines why and how passenger, crew, service-recovery and commerce data are used</strong> and therefore acts as controller. PlanetHike OÜ generally processes those data on the cruise line's documented instructions as a processor. The exact allocation of responsibilities is defined in the cruise-line agreement and deployment-specific privacy notice.</p><p>PlanetHike OÜ may act as an independent controller for limited data required for its own legal compliance, enterprise account administration, platform security, abuse prevention and defence of legal claims.</p></section>
        <section id="data"><h2>3. Categories of data</h2><p>Depending on the feature used and the cruise-line configuration, Cruise Connection may process:</p><ul><li><strong>Sailing and launch context:</strong> pseudonymous guest identifier, ship, sailing, age band, locale and feature permissions supplied by the host cruise app.</li><li><strong>Passenger profile data:</strong> chosen first name, optional profile image, interests, discoverability preferences and shared-activity context.</li><li><strong>Positive interactions:</strong> predefined affirmations, acknowledgements, blocks/reports and predefined public-place or activity proposals.</li><li><strong>Crew recognition:</strong> crew identifier, selected recognition reasons, sailing/venue context and, where used, a photograph of the crew member's badge for identification. Cruise Connection is not designed to use facial recognition.</li><li><strong>Service recovery and Experience Pulse:</strong> department satisfaction scores, issue categories, routing status, response/resolution information and post-recovery satisfaction.</li><li><strong>Commerce events:</strong> interests, recommendations shown, booking handoff events and attribution identifiers. Cruise Connection is not intended to receive full payment-card data.</li><li><strong>Proximity signals:</strong> temporary, coarse nearby-discovery identifiers where the cruise-line app/device supports this feature.</li><li><strong>Technical data:</strong> device/browser information, service-worker/cache state, security logs and offline queue records required to operate the module.</li><li><strong>Business contact data:</strong> name, work email, company, role and information included in a pilot or enterprise enquiry.</li></ul></section>
        <section id="nearby"><h2>4. Nearby discovery and privacy</h2><p>Nearby passenger discovery is designed to be <strong>opt-in</strong>. When enabled by a cruise-line deployment, the host app may use local device capabilities such as Bluetooth Low Energy or other ship-approved proximity technology to identify other opted-in passengers in coarse proximity.</p><p>The feature is designed not to expose cabin number, booking reference, phone number, exact distance, continuous GPS location or a ship-wide passenger manifest. Other passengers should see only the minimum profile information the guest has agreed to make discoverable, such as first name, selected profile image and chosen interests/context.</p><p>Nearby discovery should use short-lived or rotating identifiers and should not create a long-term movement history. Passengers can turn discoverability off. Minor accounts are excluded from peer discovery by default unless a cruise-line deployment applies an approved age-appropriate configuration.</p></section>
        <section id="purposes"><h2>5. Why data are processed and legal bases</h2><p>Depending on the context and controller, data may be processed to:</p><ul><li>provide the embedded service and verified sailing access;</li><li>enable optional nearby, interest and shared-activity discovery;</li><li>deliver structured affirmations and public-place/activity proposals;</li><li>capture crew recognition and create recognition summaries;</li><li>route private service issues and measure recovery effectiveness;</li><li>recommend relevant cruise-line activities or inventory and measure agreed attribution;</li><li>operate offline queues and synchronize actions when ship or internet connectivity returns;</li><li>prevent abuse, investigate reports and protect platform security;</li><li>respond to pilot and business enquiries and administer enterprise relationships;</li><li>meet legal, accounting and compliance obligations.</li></ul><p>Relevant GDPR legal bases may include performance of a contract, legitimate interests, compliance with legal obligations and consent for optional features where consent is the appropriate basis. Optional discoverability, optional sharing and any non-essential tracking should be presented separately and be capable of being withdrawn. In a live deployment, the cruise line's privacy notice identifies the legal bases it relies on as controller.</p></section>
        <section><h2>6. Offline operation and local storage</h2><p>Cruise Connection is designed to work even when the passenger has no public internet connection. Essential application files may be cached on the device and actions may be stored temporarily in local browser/app storage until the ship-local service or approved backend can receive them. Offline storage is used to preserve requested functionality, not for advertising.</p><p>Where the module is pre-bundled in the cruise-line app or served from the ship-local network, the cruise line may also operate technical storage governed by its own privacy notice.</p></section>
        <section id="sharing"><h2>7. Who receives data</h2><p>We do not sell personal data. Data may be made available only as necessary to:</p><ul><li>the relevant cruise-line partner and its authorized onboard teams;</li><li>PlanetHike OÜ personnel and approved service providers supporting hosting, security, communications, development or support;</li><li>payment/booking systems operated by the cruise line when a passenger chooses to follow a commerce handoff;</li><li>professional advisers, insurers, authorities or courts where required for legal compliance or claims.</li></ul><p>Public social sharing occurs only when the user chooses to share. A passenger's public affirmation summary is designed to omit the identities of senders unless those people have separately permitted identification.</p></section>
        ...
        <section id="retention"><h2>9. Retention</h2><p>We keep personal data only for as long as needed for the stated purpose, contractual obligations, security, accounting or legal claims. For live cruise deployments, the cruise line sets operational retention periods in accordance with its policies and the data-processing agreement.</p><p>Our design principle is that proximity identifiers remain short-lived, offline queues are removed after successful synchronization or expiry, and badge photographs are retained only as long as needed to identify the crew member or handle a related dispute/security issue. Aggregated or irreversibly anonymized statistics may be retained longer because they no longer identify an individual.</p></section>
        <section id="rights"><h2>10. Your rights</h2><p>Where the GDPR applies, you may have rights to access, correct, erase or restrict your personal data, receive portable data in applicable cases, object to certain processing and withdraw consent without affecting processing already lawfully carried out.</p><p>For data processed within a cruise-line deployment, the cruise line shown in the app/privacy notice is normally the first contact for exercising passenger or crew rights. For data for which PlanetHike OÜ is controller, contact <a href="mailto:hello@planethike.org">hello@planethike.org</a> with the subject “Privacy request – Cruise Connection”.</p><p>You may also lodge a complaint with the Estonian Data Protection Inspectorate (Andmekaitse Inspektsioon) or another competent supervisory authority.</p></section>
        <section><h2>11. Security</h2><p>We design Cruise Connection around data minimization, pseudonymous sailing identifiers, constrained passenger interactions, opt-in discoverability, role-based access, secure transport, limited retention and separation of positive recognition from private complaints. No system can guarantee absolute security, and live deployments require cruise-line security review and agreed technical controls.</p></section>
        <section><h2>12. Automated decision-making</h2><p>Cruise Connection may use rules or recommendations to surface relevant activities or operational signals. It is not designed to make solely automated decisions that produce legal or similarly significant effects on passengers or crew. Any material employment, safety, complaint or service decision remains with the cruise line and its authorized personnel.</p></section>
        <section><h2>13. Changes and contact</h2><p>We may update this policy as the product, cruise-line integrations or legal requirements evolve. Material changes will be reflected by the “Last updated” date and, where required, additional notice.</p><p>Controller for this website: PlanetHike OÜ, Järvevana tee 9, Tallinn, 11314, Estonia · Registry code 17233027 · <a href="mailto:hello@planethike.org">hello@planethike.org</a>.</p></section>
      </article></div>
    </main>
  )
  ...
  if (kind === 'terms') return (
    <main className="legal-page">
      <div className="legal-hero"><div className="section-kicker">TERMS</div><h1>Terms of Service</h1><p>Terms for the Cruise Connection website, demonstrations and partner-deployed passenger module.</p><span>Last updated: {updated}</span></div>
      <div className="legal-layout"><aside className="legal-toc"><b>Terms</b><a href="#scope">Scope</a><a href="#module">Passenger module</a><a href="#conduct">Interaction rules</a><a href="#commerce">Commerce</a><a href="#liability">Liability</a><a href="#law">Governing law</a></aside><article className="legal-content">
        <section id="scope"><h2>1. Operator and scope</h2><p>These Terms govern access to the Cruise Connection website, demonstrations and, where enabled by a cruise-line partner, the Cruise Connection passenger-facing and operational module. Cruise Connection is operated by PlanetHike OÜ, registry code 17233027, Järvevana tee 9, Tallinn, 11314, Estonia.</p><p>For cruise-line customers, a signed enterprise, pilot, data-processing or licensing agreement prevails over these public Terms to the extent of any conflict.</p></section>
        <section><h2>2. White-label service</h2><p>Cruise Connection is designed to sit within or alongside a cruise line's existing digital environment. The cruise line remains responsible for its passenger contract, vessel operations, safety, staff management, itinerary, activities, inventory, pricing, bookings, payments and fulfilment. Cruise Connection does not itself operate a vessel, excursion, restaurant, spa, casino, medical service or other physical cruise service.</p><p>Unless expressly stated, demo brands, ships, people, statistics and commercial figures are illustrative and do not indicate a partnership with a particular cruise line.</p></section>
        ...
        <section id="conduct"><h2>4. Positive-interaction rules</h2><p>Cruise Connection intentionally limits passenger-to-passenger communication. It is not an unrestricted chat service and is not presented as a dating service.</p><ul><li>First contact uses approved predefined positive affirmations.</li><li>The recipient may acknowledge, ignore, block or report.</li><li>A meeting/activity proposal is available only after the recipient responds positively.</li><li>Meeting proposals offered by Cruise Connection are limited to public onboard venues or activities. Cabins/staterooms are not offered as meeting locations.</li><li>Users must respect another person's decision not to respond or to block/report.</li><li>Users must not use the service for harassment, stalking, discrimination, sexual solicitation, threats, scams, commercial spam or attempts to obtain private passenger/crew information.</li></ul><p>Physical interaction remains voluntary. A positive response in the app is not consent to physical contact, sexual activity, purchases or any activity beyond the specific structured interaction shown.</p></section>
        <section><h2>5. Nearby and interest discovery</h2><p>Nearby discovery is intended for passengers who voluntarily enable discoverability. It provides coarse proximity, not a precise tracking map. You must not use proximity features to follow, monitor or locate another person after they have disabled discoverability or indicated that they do not wish to interact.</p><p>Shared interests are self-selected and may be used to surface passengers, activities or cruise-line offers that are relevant to those interests. Cruise Connection does not guarantee that another user's profile, interest or availability remains current.</p></section>
        <section><h2>6. Crew recognition</h2><p>Crew recognition is for genuine positive service interactions. In the standard design, a passenger may recognize the same crew member once per sailing-local day and may select up to two predefined recognition reasons for that interaction. Artificial, coordinated or fraudulent recognition may be removed or excluded from reporting.</p><p>Recognition summaries are not intended to be public employee rankings or star-rating systems. Employment, compensation, performance and disciplinary decisions remain solely with the cruise line.</p></section>
        <section><h2>7. Service recovery and Experience Pulse</h2><p>Private service-recovery tools and department satisfaction prompts are intended to help the cruise line identify and address issues during the sailing. They are not emergency services. For medical emergencies, personal safety threats, missing persons, fire, crime or other urgent situations, passengers must use the cruise line's designated emergency or onboard assistance procedures.</p><p>Submitting an issue does not guarantee a particular remedy, refund, response time or outcome. Those decisions belong to the cruise line under its passenger contract and policies.</p></section>
        <section id="commerce"><h2>8. Activities, recommendations and commerce</h2><p>Cruise Connection may surface relevant dining, excursions, spa, retail, entertainment or other cruise-line inventory based on shared interests, group context or availability. A recommendation is not a guarantee of availability, quality, suitability or price.</p><p>Unless expressly agreed otherwise, the cruise line or its designated merchant handles inventory, pricing, checkout, payment, refunds and fulfilment. Their booking and passenger terms apply to those transactions. Cruise Connection may record referral or attribution events used to measure incremental commerce under the cruise-line agreement.</p></section>
        <section><h2>9. Social sharing</h2><p>Where sharing is offered, you choose whether to export or share your positive-memory or recognition summary. You are responsible for the third-party social platform you choose to use and its terms. Cruise Connection is designed not to publish the identities of affirmation senders in a recipient's public summary unless separate permission has been given.</p></section>
        <section><h2>10. Minors</h2><p>Minor access must follow the cruise line's account, consent and safeguarding rules. Peer discovery for minors is disabled by default in the Cruise Connection design. A cruise-line partner must expressly configure and approve any age-appropriate functionality made available to minors.</p></section>
        <section><h2>11. Intellectual property</h2><p>The Cruise Connection software, design, documentation, branding, GAMMESO framework and original content are owned by PlanetHike OÜ or its licensors and are protected by applicable intellectual-property laws. No licence is granted except the limited right to access the service for its intended purpose. Reverse engineering, scraping, copying, resale or creation of derivative commercial services is prohibited except where applicable law expressly permits it.</p></section>
        ...
        <section><h2>17. Contact and changes</h2><p>We may update these Terms as the product or legal requirements evolve. Continued use after an effective update may be subject to the updated Terms where permitted by law.</p><p>Questions: <a href="mailto:hello@planethike.org">hello@planethike.org</a> · PlanetHike OÜ, Järvevana tee 9, Tallinn, 11314, Estonia.</p></section>
      </article></div>
    </main>
  )
  ...
  return (
    <main className="legal-page">
      <div className="legal-hero"><div className="section-kicker">STORAGE & COOKIES</div><h1>Cookie & Local Storage Policy</h1><p>What the current Cruise Connection website stores on your device—and what it does not.</p><span>Last updated: {updated}</span></div>
      <div className="legal-layout"><aside className="legal-toc"><b>Storage</b><a href="#current">Current use</a><a href="#essential">Essential storage</a><a href="#nonessential">Analytics & marketing</a><a href="#manage">Manage storage</a></aside><article className="legal-content">
        ...
        <section><h2>6. Changes and contact</h2><p>We will update this page if the technologies used by Cruise Connection change. If a future change requires consent, the consent mechanism will be introduced before the relevant non-essential technology is activated where required by law.</p><p>Questions: <a href="mailto:hello@planethike.org">hello@planethike.org</a>.</p></section>
      </article></div>
    </main>
  )
}
