...
export default function Sandbox(){
  ...
  return <main className="sandbox-shell">
    ...
    <section className="sandbox-grid">
      ...
      <article className="sandbox-phone"><div className="phone-top"><span>9:41</span><b>Oceanic Aurora</b><span>●●●</span></div><div className="phone-body"><p>CRUISECONNECT</p><h2>{featureCards.find(x=>x[0]===active)?.[1]}</h2><div className="scenario-card"><span>Live sailing · Day 2</span><strong>{featureCards.find(x=>x[0]===active)?.[2]}</strong><small>Adult guest · feature entitled · operator-local identity</small></div>{active==='offline'&&queue>0?<button className="primary" onClick={reconnect}>Restore connection & sync</button>:<button className="primary" onClick={run}>Run this journey</button>}<small className="privacy-note">Identity, captures and source actions remain inside this reference host.</small></div></article>
      <aside><div className="sandbox-kpis"><div><span>Host actions</span><b>{counts.actions}</b></div><div><span>Queued</span><b>{queue}</b></div><div><span>Notifications</span><b>{counts.notifications}</b></div><div><span>Revenue</span><b>€{counts.revenue}</b></div></div><h2>Operational event stream</h2>{logs.length?logs.map((x,i)=><div className="log" key={`${x.time}-${i}`}><time>{x.time}</time><p><b>{x.title}</b><span>{x.detail}</span></p></div>):<p className="empty">Run a journey to see idempotent host-side activity.</p>}</aside>
    </section>
  </main>
}
